...
static int led_set_pwm(int pwm_num, int level)
{
	...
	pwm_setting.clk_div = CLK_DIV1; 		
		...
		pwm_setting.PWM_MODE_OLD_REGS.DATA_WIDTH = 4; // 256 level
	//	pwm_setting.PWM_MODE_OLD_REGS.THRESH = level;
    
	if(level)
	{
		pwm_setting.PWM_MODE_OLD_REGS.THRESH = pwm_setting.PWM_MODE_OLD_REGS.DATA_WIDTH/level;
	...
	printk("[LEDS]LK: brightness_set_pwm: level=%d, clk=%d \n\r", level, pwm_setting.clk_src);
...
int FL_Init(void)
{
    ...
    /*Init. to disable*/
    if(mt_set_gpio_mode(GPIO_ENT,GPIO_MODE_02)){PK_DBG("[constant_flashlight] set gpio mode failed!! \n");}
