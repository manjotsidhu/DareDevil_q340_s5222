...
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    #if GTP_SLIDE_WAKEUP
	if (tpd_halt && doze_status == DOZE_WAKEUP) {
            doze_status = DOZE_ENABLED;
        }
        if (DOZE_ENABLED == doze_status)
        {
            ret = gtp_i2c_read(i2c_client_point, doze_buf, 6);
            GTP_DEBUG("0x814B = 0x%02X", doze_buf[2]);
            GTP_DEBUG("doze_buf[4] = 0x%02X", doze_buf[4]);
            GTP_DEBUG("doze_buf[5] = 0x%02X", doze_buf[5]);
            ...
            {  
                ...
                {
                	if (((doze_buf[4] == 0x01) ||(doze_buf[4] == 0x02)||(doze_buf[4] == 0x04)) && (doze_buf[5] == 0x00))
                	{
                        GTP_INFO("Ignore button key double click!");
                        // clear 0x814B
                        doze_buf[2] = 0x00;
                        gtp_i2c_write(i2c_client_point, doze_buf, 3);
                	}
