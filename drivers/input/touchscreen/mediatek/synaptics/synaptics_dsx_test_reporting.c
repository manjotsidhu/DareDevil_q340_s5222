		...
		struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to read control reg_18\n",__func__);
	}
	...
	for (ii = 0; ii < f54->control.reg_17->length; ii++) {
		...
		if (retval < 0) {
			printk(&rmi4_data->i2c_client->dev,"%s: Faild to write output\n",__func__);
			return retval;
		}
		size += retval;
		temp += retval;
	...
	retval = snprintf(temp, PAGE_SIZE - size, "\n");
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Faild to write null terminator\n",__func__);
		return retval;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (count < f54->report_size) {
		printk(&rmi4_data->i2c_client->dev,"%s: Report type %d data size (%d) too large\n",__func__, f54->report_type, f54->report_size);
	...
	if (f54->report_data) {
		...
		printk(&rmi4_data->i2c_client->dev,"%s: Report type %d data not available\n",__func__, f54->report_type);
		mutex_unlock(&f54->data_mutex);
		return -EINVAL;
	}
}
...
static int synaptics_rmi4_f54_set_sysfs(void)
{
	...
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to create sysfs attributes\n",__func__);
		goto exit_3;
	}
