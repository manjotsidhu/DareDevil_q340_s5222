		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
		...
		printk(&rmi4_data->i2c_client->dev,
				"%s: I2C retry %d\n",
				__func__, retry + 1);
		msleep(20);
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			{   
				tpd_button(x, y, 1);  
                                printk("==================!NORMAL_BOOT=========x=%d, y=%d=====boot_mode=%d=============down=======NORMAL_BOOT%d==============\n",x,y,boot_mode,NORMAL_BOOT);  
			}	
#endif
			printk(&rmi4_data->i2c_client->dev,
					"%s: Finger %d:\n"
	...
	if (touch_count == 0) {
		...
		{   
			tpd_button(x, y, 0); 
        printk("==================!NORMAL_BOOT=========x=%d, y=%d=====boot_mode=%d============up========NORMAL_BOOT%d==============\n",x,y,boot_mode,NORMAL_BOOT);  
		}   
#endif
	}
...
//	    ps_status = APDS9930_get_ps_status();   by zhangxian
        ...
        else if(detected_gestures[0]==0x0B)
       {
		...
		{
		            printk("===synaptics_rmi4_f12_abs_report=====ic%d================\n",detected_gestures[2]);
                case 0x63:
                    gTGesture ='c';
	...
	printk(&rmi4_data->i2c_client->dev,"%s: Number of fingers to process = %d\n",__func__, fingers_to_process);
	}
		...
		if ((!Delicacy_status&&finger_status) | (Delicacy_status &&finger_Delicacy_status)) 
	{
		    ...
		    if(Delicacy_status &&(finger_Delicacy_status==6)&&(x>=10&&y>=110)&&(x<=700&&y<=525)) 
		      {
			printk("==================tpd_down==========Delicacy_status====================finger=%d=x%d=y%d====\n",finger,x,y);
...
{                     if(old_y >1100)
	...
	{
		printk("==================tpd_up==========Delicacy_status=====finger==%d=======x%d=y%d=\n",finger,x,y);
		int ifnum =0;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	printk(&rmi4_data->i2c_client->dev,"%s: Function %02x reporting\n",__func__, fhandler->fn_number);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to alloc mem for function handle\n",__func__);
		return -ENOMEM;
	}
	...
	if (!f1a->button_control.txrx_map) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to alloc mem for tx rx mapping\n",__func__);
		return -ENOMEM;
	}
...
static int synaptics_rmi4_f1a_button_map(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *fhandler)
{     
        printk("========================synaptics_rmi4_f1a_button_map=============start======================");
	int retval;
	unsigned char ii;
	...
	if (cap_button_map.map) {
	 ...
	 printk("========================synaptics_rmi4_f1a_button_map=============end======================");
	return 0;
}
		...
		bool *was_in_bl_mode)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(intr_status));
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to read interrupt status\n",__func__);
		return retval;
	}
...
static void synaptics_rmi4_set_configured(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(device_ctrl));
	if (retval < 0) {
		printk(&(rmi4_data->input_dev->dev),"%s: Failed to set configured\n",__func__);
		return;
	}
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			if (rmi_fd.fn_number == 0) {
				printk(&rmi4_data->i2c_client->dev,"%s: Reached end of PDT\n",__func__);
				break;
			}

			printk(&rmi4_data->i2c_client->dev,"%s: F%02x found (page %d)\n",__func__, rmi_fd.fn_number,page_number);

			switch (rmi_fd.fn_number) {
			case SYNAPTICS_RMI4_F01:
				printk("====LGC====SYNAPTICS_RMI4_F01====\n");
				...
				retval = synaptics_rmi4_alloc_fh(&fhandler,
						&rmi_fd, page_number);
				if (retval < 0) {
					printk(&rmi4_data->i2c_client->dev,"%s: Failed to alloc for F%d\n",__func__,rmi_fd.fn_number);
					return retval;
				}
				...
				if (retval < 0) {
					printk(&rmi4_data->i2c_client->dev,"%s: Failed to check status\n",__func__);
					return retval;
				}
...
flash_prog_mode:
	...
	if (rmi->manufacturer_id != 1) {
		printk(&rmi4_data->i2c_client->dev,"%s: Non-Synaptics device found, manufacturer ID = %d\n",__func__, rmi->manufacturer_id);
	}
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmi4_data->input_dev == NULL) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to allocate input device\n",__func__);
		retval = -ENOMEM;
	...
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to query device\n",__func__);
	...
	if (retval) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to register input device\n",__func__);
...
static int synaptics_rmi4_reinit_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			printk(&rmi4_data->i2c_client->dev,"%s: Interrupt enable mask %d = 0x%02x\n",__func__, ii, rmi4_data->intr_mask[ii]);
			intr_addr = rmi4_data->f01_ctrl_base_addr + 1 + ii;
			retval = synaptics_rmi4_i2c_write(rmi4_data,
					intr_addr,
					&(rmi4_data->intr_mask[ii]),
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to issue reset command, error = %d\n",__func__, retval);
		mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
		return retval;
	}
...
#endif
{
	...
	if (!i2c_check_functionality(client->adapter,
		...
		printk(&client->dev,"%s: SMBus byte data not supported\n",__func__);
		return -EIO;
	}
...
*/
	...
	if (!rmi4_data) {
		printk(&client->dev,"%s: Failed to alloc mem for rmi4_data\n",__func__);
		return -ENOMEM;
	}
	...
	//yaohua.li
         ...
         if (misc_register(&rmi4_data->firmware) < 0)
                   printk("mtk-tpd:[SY] misc_register failed!!\n");
         else
                   printk("[SY] misc_register finished!!\n"); 
...
//	  strcpy(Tg_buf,"mcs");
	...
	{
		printk("[STP]SY_key_dev register : fail!\n");
	}else
	{
		printk("[STP]SY_key_dev register : success!!\n");
	} 
//end by yaohua.li
		...
		bool enable)
{
	...
	printk("=====synaptics_rmi4_f12_wg===start===\n");
	struct synaptics_rmi4_f12_extra_data *extra_data;
	struct synaptics_rmi4_fn *fhandler;
	struct synaptics_rmi4_device_info *rmi;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
	retval = synaptics_rmi4_i2c_write(rmi4_data,fhandler->full_addr.ctrl_base + offset,reporting_control,
			sizeof(reporting_control));
	if (retval < 0) {
		printk(&(rmi4_data->input_dev->dev),"========%s: Failed to change reporting mode\n",__func__);
		return;
	}
//yaohua.li
printk("=====synaptics_rmi4_f12_wg===end===\n");
	return;
}
...
static void synaptics_rmi4_wakeup_gesture(struct synaptics_rmi4_data *rmi4_data,
		bool enable)
{
printk("====synaptics_rmi4_wakeup_gesture===rmi4_data->f11_wakeup_gesture==%d==rmi4_data->f12_wakeup_gesture%d=\n",rmi4_data->f11_wakeup_gesture,rmi4_data->f12_wakeup_gesture);
	if (rmi4_data->f11_wakeup_gesture)
...
{    
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
	     if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
	     {
		printk("=======set_tgesture_enable======find====\n");
			break;
             }
 	}
	       extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
...
{    
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
	     if (fhandler->fn_number == SYNAPTICS_RMI4_F12)
	     {
		printk("=======et_tgesture_unenable======find====\n");
			break;
             }
 	}
	       extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
...
void set_sleep_unenable()
{
	...
	list_for_each_entry(fhandler, &rmi->support_fn_list, link) 
	{
		...
		printk("===========synaptics_rmi4_f12_abs_report==set_sleep===================true start====\n");
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (rmi4_data->staying_awake)
		return 0;
	printk("============synaptics_rmi4_suspend===%d====\n",rmi4_data->enable_wakeup_gesture);
	//bTPGesturetemp =bEnTGesture;
      //  printk("============synaptics_rmi4_suspend==bEnTGesture=%d====\n",bEnTGesture);
	if (bTPGesturetemp) {	
	synaptics_chip_reset();
	set_tgesture_enable();
	if (rmi4_data->enable_wakeup_gesture) {
 ...
 hwPowerDown(TPD_POWER_SOURCE, "TP");
    msleep(200);
		...
		printk("============synaptics_rmi4_resume===set_tgesture_unenable==\n");	
	}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (rmi4_data->staying_awake)
		return 0;
        printk("============synaptics_rmi4_resume===%d====\n",rmi4_data->enable_wakeup_gesture);
	synaptics_chip_reset();	
	if (bTPGesturetemp) 
	{	
		...
		if (rmi4_data->enable_wakeup_gesture) 
		{		
		printk("====rmi4_data->enable_wakeup_gesture===%d==\n",rmi4_data->enable_wakeup_gesture);
		synaptics_rmi4_wakeup_gesture(rmi4_data, false);
		goto exit;
		}
	}
	...
	{
		//synaptics_chip_reset();
		printk("============synaptics_rmi4_resume====set_tgesture_enable==\n");
	}
	synaptics_rmi4_sensor_wake(rmi4_data);
	synaptics_rmi4_irq_enable(rmi4_data, true);
	retval = synaptics_rmi4_reinit_device(rmi4_data);
...
char* get_synaptics_fw_name(void)
{
	...
	printk("===synaptics=======fw==name");
	strcpy(buf, bufsy);
	return buf;
}
char* get_synaptics_IC_name(void)
{
	char bufsy[32]= {"s3508r"};
	static char buf[32]= {0};
	printk("===synaptics=======IC==name");
	strcpy(buf, bufsy);
	return buf;
}
...
char* get_synaptics_TpName( void )
{
   ...
   sprintf(tpd_desc_info, "%s-L5560AE-%s-0x%x",get_synaptics_fw_name(),get_synaptics_IC_name(),get_synaptics_versionid());
   printk("=======tpd_desc_info=%s===",tpd_desc_info);
