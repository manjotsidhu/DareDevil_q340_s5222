		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		printk(&rmidev->rmi4_data->i2c_client->dev,"%s: Out of register map limit\n",__func__);
		return -EINVAL;
	}
	...
	if (length) {
		retval = rmidev->fn_ptr->read(rmidev->rmi4_data,
				...
				length);
		if (retval < 0) {
			printk(&rmidev->rmi4_data->i2c_client->dev,"%s: Failed to read data\n",__func__);
			return retval;
		}
	} else {
		return -EINVAL;
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev->fn_ptr->write(rmidev->rmi4_data,
				...
				length);
		if (retval < 0) {
			printk(&rmidev->rmi4_data->i2c_client->dev,"%s: Failed to write data\n",__func__);
			return retval;
		}
	} else {
		return -EINVAL;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(&rmi4_data->i2c_client->dev,"%s: Attention interrupt disabled\n",__func__);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(&rmi4_data->i2c_client->dev,"%s: Attention interrupt enabled\n",__func__);
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		printk(&rmidev->rmi4_data->i2c_client->dev,
...
static int rmidev_open(struct inode *inp, struct file *filp)
{
	...
	printk(&rmidev->rmi4_data->i2c_client->dev,"%s: Attention interrupt disabled\n",__func__);
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		printk(&rmidev->rmi4_data->i2c_client->dev,"%s: rmidev device removed\n",__func__);
	}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (rmidev_major_num) {
		...
		printk(&rmi4_data->i2c_client->dev,"%s: Major number of rmidev = %d\n",__func__, rmidev_major_num);
		
	}
	
	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to alloc mem for dev_data\n",__func__);
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to add rmi char device\n",__func__);
	...
	if (IS_ERR(device_ptr)) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to create rmi char device\n",__func__);
		retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (!rmidev->sysfs_dir) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to create sysfs directory\n",__func__);
		retval = -ENODEV;
		goto err_sysfs_dir;
	}
	...
	retval = sysfs_create_bin_file(rmidev->sysfs_dir,
			&attr_data);
	if (retval < 0) {
		printk(&rmi4_data->i2c_client->dev,"%s: Failed to create sysfs bin file\n",__func__);
		goto err_sysfs_bin;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			printk(&rmi4_data->input_dev->dev,"%s: Failed to create sysfs attributes\n",__func__);
			retval = -ENODEV;
